...
/**
 * Admin panel browser-tab icon. This is the standardized WCM admin mark (a
 * fixed template asset shipped with the panel, same across every Tentakel
 * site's cms-admin — see includes/sidebar.php), deliberately independent
 * of Site Settings — site_settings.logo_path/favicon_path is the PUBLIC
 * site's brand, a completely different identity from the admin panel's
 * own mark. An earlier version of this function pulled from site_settings
 * instead, which incorrectly showed the public site's logo as the admin
 * panel's tab icon.
 *
 * cms-admin/assets/img/logo.png already IS the correct mark (black,
 * transparent background — the black variant reads better than white
 * against the light tab chrome most browsers use) — same asset used for
 * the sidebar brand on the light theme — so the favicon just reuses it via
 * cms_asset_url(), same as every other admin asset.
 */
