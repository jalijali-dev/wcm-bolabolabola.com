...
// Prompt Control depends on the optional services/PromptLoader.php file
// (not present on every environment — see feature-unavailable.php, used
// by prompt-control*.php when it's missing). Flag it here so the sidebar
// link can carry a small "Belum Tersedia" badge instead of looking like
// every other fully-working link.
$cmsPromptControlAvailable = is_file(dirname(__DIR__, 2) . '/services/PromptLoader.php');
    ...
    <div class="admin-sidebar__brand">
        <a class="admin-sidebar__logo" href="<?= cms_esc(cms_dashboard_href()) ?>">
            <!-- img/logo.png (black) + img/logo-white.png (white) are the
                 standardized WCM admin mark (16 Agu 2026 — same mark across
                 every Tentakel site's cms-admin, only CMS_ADMIN_TAGLINE
                 differs per site), already theme-aware via the
                ...
                <div class="admin-sidebar__group">
                    ...
                    <?php foreach ($section['items'] as $item) :
                        ...
                        <a class="admin-navlink<?= $active ? ' is-active' : '' ?>" href="<?= cms_esc($item['href']) ?>" data-icon="<?= cms_esc($item['icon']) ?>">
                            ...
                            <?php if ($item['id'] === 'prompt-control' && !$cmsPromptControlAvailable) : ?>
                                <span class="admin-navlink__badge" title="services/PromptLoader.php belum ter-install di environment ini">Belum Tersedia</span>
                            <?php endif; ?>
                        </a>
                    <?php endforeach; ?>
                </div>
